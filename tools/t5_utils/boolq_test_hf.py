...
def preds_to_text(tokenizer, preds, eos_token_id):
...
def mnli_task(args_dict):
    ...
    vocab_path = ckpt_path / "tokenizer.model"
    ...
    tokenizer_t5 = get_nmt_tokenizer(
                    ...
                    model_name=None,
                    tokenizer_model=vocab_path.as_posix(),
                    vocab_file=None,
                    merges_file=None,
                    legacy=True,
                )   
    bos_token_id = ckpt_config.getint("decoder", "decoder_start_token_id")
    ...
    with create_inference_server_client(args_dict["protocol"],
        ...
        while current_idx < len(boolq_test_data):
            ...
            for data in batch_boolq_test_data:
                text = data['passage'] + " Question: " + data['question'] + " true or false?"
            ...
            for i in range(len(batch_tokens)):
                batch_tokens[i] = torch.cat((prompt_ids,
                                            ...
                                            bos_token_id * torch.ones([1, max_length - len(batch_tokens[i])]).to(torch.int32)), axis=1) # HF
            ...
            start_ids = bos_token_id * np.ones([input_ids.shape[0], 1]).astype(np.uint32)
            ...
            if args_dict["use_request_prompt_embedding"]:
                ...
                request_prompt_embedding = np.array([prompt_weights_dict[t] for t in task_names]).astype(np.float32)
        ...
        for result in results:
            ...
            preds = preds_to_text(tokenizer_t5, torch.IntTensor(ft_decoding_outputs), eos_token_id)
            preds_list += preds
