...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        for idx, request in enumerate(requests):
            ...
            print(query[0, ...])
            input_id, request_input_len = self._create_request(query[0, ...])
            bad_words = to_word_list_format(bad_words_dict[0, ...])
            stop_words = to_word_list_format(stop_words_dict[0, ...])

            # Create output tensors. You need pb_utils.Tensor
            # objects to create pb_utils.InferenceResponse.
            input_id_tensor = pb_utils.Tensor(
                'INPUT_ID',
                np.expand_dims(np.array(input_id).astype(self.input_id_dtype), axis=0))
            request_input_len_tensor = pb_utils.Tensor(
                'REQUEST_INPUT_LEN',
                np.expand_dims(np.array(request_input_len).astype(self.request_input_len_dtype), axis=0))
            request_output_len_tensor = pb_utils.Tensor(
                'REQUEST_OUTPUT_LEN',
                request_output_len)
            bad_words_ids_tensor = pb_utils.Tensor(
                'BAD_WORDS_IDS',
                np.expand_dims(bad_words, axis=0))
            stop_words_ids_tensor = pb_utils.Tensor(
                'STOP_WORDS_IDS',
                np.expand_dims(stop_words, axis=0))
